...
const DialogCardColection = ({ data, image, isLiked, handleLike, disabled }) => {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button
          ...
          title="Suka"
          size="sm"
          className="relative group/button flex gap-2 items-center justify-center px-4 rounded-md border-2 hover:pr-8 transition-all duration-300"
        >
          <div className="flex items-center">
            <p>Selengkapnya</p>

            <ArrowRight
              className="absolute  right-2 opacity-0 group-hover/button:opacity-100 transition-opacity duration-300"
              size={16}
            />
          </div>
        </Button>
      ...
      <DialogContent className="px-4 py-4 max-w-screen-sm max-h-[95vh] sm:max-h-[90vh] overflow-y-auto no-scrollbar bg-green-shades-95 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center w-full px-2">
          ...
          <DialogFooter className={""}>
            <LikeButton isLiked={isLiked} onClick={() => handleLike(data.herbId, image)} disabled={disabled} />
          </DialogFooter>
        </div>
      </DialogContent>
    </Dialog>
...
export default DialogCardColection;
